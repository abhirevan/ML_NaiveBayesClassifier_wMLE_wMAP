...
class NaiveBayesClassifier(object):
    ...
    def predict(self, x_df, y_truth):
        ...
        for _, row in x_df.iterrows():
            ...
            pr[0] = log(self.class_pr[0])
            pr[1] = log(self.class_pr[1])
            ...
            for i, wc in enumerate(row):
                ...
                pr[0] = pr[0] + log((c_wi_0 + self.alpha - 1) / float(self.n_D[0] + self.n_words * (self.alpha - 1)))
                pr[1] = pr[1] + log((c_wi_1 + self.alpha - 1) / float(self.n_D[1] + self.n_words * (self.alpha - 1)))
